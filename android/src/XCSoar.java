...
public class XCSoar extends Activity implements PermissionManager {
  ...
  public void initNative() {
    ...
    top.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, 0));
    top.setTag("TopInset");
    ...
    parent.setOnApplyWindowInsetsListener(new View.OnApplyWindowInsetsListener() {
      ...
      public WindowInsets onApplyWindowInsets(View v, WindowInsets insets) {
          ...
          if(android.os.Build.VERSION.SDK_INT >= 30) {
            topInsetSize = insets.getInsets(WindowInsets.Type.systemBars()).top;
          } else {
            topInsetSize = insets.getSystemWindowInsetTop();
          }
          View top = ((ViewGroup) v).findViewWithTag("TopInset");
          top.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, topInsetSize));

          int bottomInsetSize = 0;
          if(android.os.Build.VERSION.SDK_INT >= 30) {
            bottomInsetSize = insets.getInsets(WindowInsets.Type.systemBars()).bottom;
          } else {
            bottomInsetSize = insets.getSystemWindowInsetBottom();
          }
          View bottom = ((ViewGroup) v).findViewWithTag("BottomInset");
          bottom.setLayoutParams(new LayoutParams(LayoutParams.MATCH_PARENT, bottomInsetSize));
        return insets;
      }
    });
