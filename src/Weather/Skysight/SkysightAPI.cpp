...
SkysightAPI::OnTimer()
{
  AllocatedPath regions_path = GetPath(SkysightCallType::Regions);
  AllocatedPath layers_path = GetPath(SkysightCallType::Layers);

  auto regions_tp = File::GetLastModification(regions_path);
  auto layers_tp = File::GetLastModification(layers_path);

  auto now = std::chrono::system_clock::now();
  uint64_t now_tt = std::chrono::system_clock::to_time_t(BrokenDateTime::NowUTC().ToTimePoint());

  // Bug handling when device time is not accurate, might be RTC battery is dead
  auto d1 = std::chrono::duration_cast<std::chrono::seconds>(now - regions_tp);
  auto d2 = std::chrono::duration_cast<std::chrono::seconds>(now - layers_tp);

  if(d1 > std::chrono::years(1) ||
    d2 > std::chrono::years(1) ||
    std::any_of(metrics.begin(), metrics.end(), [&now_tt](auto m) {
      return (now_tt - m.last_update) > 365*24*60*60;
    }))
  {
    // To avoid DDOS request to server, we gonna stop this functionality of device with the wrong time
    timer.Cancel();
  }
  ...
  if (File::Exists(regions_path) && ((uint64_t)std::chrono::system_clock::to_time_t(regions_tp + std::chrono::days(1)) < now_tt))
    GetData(SkysightCallType::Regions, nullptr, true);

  //refresh layers cache file if > 24h old
  if (File::Exists(layers_path) && ((uint64_t)std::chrono::system_clock::to_time_t(layers_tp + std::chrono::days(1)) < now_tt))
    GetData(SkysightCallType::Layers, nullptr, true);

  //refresh last update times if > 5h (update freq is usually 5 hours)
  for (auto &m: metrics) {
    if ((m.last_update + 5*60*60) < now_tt) {
