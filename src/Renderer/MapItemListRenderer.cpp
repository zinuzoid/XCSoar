     ...
     const WindBarbLook &look)
{
  ...
  const JETProvider::WindStation &station = item.station;

  const auto age = duration_cast<seconds>(
      std::chrono::system_clock::now() - station.measured_at);
  const bool stale = age > seconds{JET_PROVIDER_WIND_STALE_THRESHOLD_SECS};
     ...
     const MapSettings &settings)
{
  ...
  if (traffic != nullptr) {
    const bool vario_traffic = settings.use_vario_traffic_colours;
    const TrafficClimbAltIndicators indicators =
      TrafficClimbAltIndicators::GetClimbAltIndicators(*traffic);
    TrafficRenderer::Draw(canvas, traffic_look, false, vario_traffic,
                          *traffic, traffic->track,
                          item.color, pt, indicators);
  }
     ...
     const MapSettings &settings)
{
  ...
  if (item.altitude >= 0)
    info_string.AppendFormat(_T(", %s: %s"), _("Alt"),
  ...
  if (item.speed >= 0)
    info_string.AppendFormat(_T(", %s: %s"), _("Spd"),
                             FormatUserSpeed(item.speed).c_str());

  if (item.epoch > 0) {
    const auto age = duration_cast<seconds>(
        std::chrono::system_clock::now() -
        std::chrono::system_clock::from_time_t(item.epoch));
    if (age.count() >= 0)
      info_string.AppendFormat(_T(", %s %s"),
                               FormatTimespanSmart(age).c_str(), _("ago"));
  }
